...
// Build is a new and improved RDF conversion
func Build(mc *minio.Client, prefix string, v1 *viper.Viper) error {
	...
	for object := range mc.ListObjectsV2(bucketname, prefix, isRecursive, doneCh) {
		...
		go func(object minio.ObjectInfo) {
			...
			_, err := obj2RDF(bucketname, "propg", mc, object, proc, options)
			if err != nil {
	...
	millprefix := strings.ReplaceAll(prefix, "summoned", "propg")
	sp := strings.SplitAfterN(prefix, "/", 2)
	mcfg := v1.GetStringMapString("gleaner")
	rslt := fmt.Sprintf("results/%s/%s_graph.lpg", mcfg["runid"], sp[1])
...
func obj2RDF(bucketname, prefix string, mc *minio.Client, object minio.ObjectInfo, proc *ld.JsonLdProcessor, options *ld.JsonLdOptions) (string, error) {
	...
	// at this point we have nquads..   need to convert to LPG

	milledkey := strings.ReplaceAll(key, ".jsonld", ".lpg")
