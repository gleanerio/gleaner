...
func main() {
	// Read file
	//dat, err := os.ReadFile("./data/cioos.json")
	dat, err := os.ReadFile("./data/sprep.json")
	if err != nil {
...
func fixContextUrl(jsonld string) (string, error) {
	...
	if !ok {
		...
		if strings.Contains(c.Value().(string), "http://scheme.org/") {
			if (sdoc == "http://schema.org/") {
				return jsonld, nil  // all good, return
			} else {
				strings.Replace(c.Value().(string), "http://schema.org/", sdoc, 1)
				return jsonld, nil
			}
		} else if (strings.Contains(c.Value().(string), "https://scheme.org/") ) {
			if (sdoc == "https://schema.org/") {
				return jsonld, nil  // all good, return
			} else {
				strings.Replace(c.Value().(string), "https://schema.org/", sdoc, 1)
				return jsonld, nil
			}
		}
	...
	for k, v := range cm {
		...
		if strings.HasPrefix(v.(string), "https://schema.org") {
			...
			if v.(string) == sdoc {
				...
				fmt.Printf("CHECK MARK:  _%s_  _%s_", tns, sdoc)
				jsonld, err = sjson.Set(jsonld, tns, sdoc)
				return jsonld, err
			}
		}
